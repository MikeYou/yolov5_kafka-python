...
"""controling conveyor and spinning bottle / jetson nano 2GB """
import board
import digitalio
import busio
import time
...
os.environ['KAFKA_HOST'] = '[flask server ip]'
producer = KafkaProducer(
        bootstrap_servers="[flask server ip]:9093",
        api_version=(2,8,1))
topic = 'TopicA'
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            data = cv2.imencode('.jpeg', im0)[1].tobytes()
